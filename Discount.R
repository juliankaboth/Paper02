...
### Select pricing for shareholdings - Remark: PPS is averaged! 
data_prce <- data %>%
             select(rlvt_prce) %>%
             group_by(ShareClassIndex, InvestorID) %>%
             summarize(PPS=mean(PPS)) %>% #PPS is averaged!
...
### Evaluate "new-investor" participation
data_fmly <- data_shar[,-1]*0+1
data_fmly[is.na(data_fmly)] <- 0
data_fmly_eval <- rep(1, nrow(data_fmly))

tmp1 <- data_fmly*0

for (i in 1:nrow(data_fmly)){
  for (j in 1:ncol(data_fmly)){
    tmp1[i,j] <- ifelse(sum(data_fmly[1:i,j])==0, 0, ifelse(sum(data_fmly[1:i,j])==1, 1, 2))
  }
}

for (i in 1:nrow(data_fmly)){
  data_fmly_eval[i] <- ifelse(any(tmp1[i,]==2, na.rm=TRUE), ifelse(any(tmp1[i,]==1, na.rm=TRUE), 3, 2), 1) # 1 indicates "old" investor, 2 indicates "new" investor, 3 indicates "new" and "old" investors
}
rm(tmp1)
...
val <- temp_valu #pull(pstm_valu, totl_shar) # Exit value of the company, corresponds to the exit proceeds - equate to the pmv of the latests round
rfr <- rf # Risk free rate
sig <- sg # Volatility of val

### Create scenario summary (for output list)
scnr <- list(Company=cy, TimeToExit=te, RiskFreeRate=rf, Volatility=sg)
...
### Weighting shares
rwsm <- rowSums(data_shar[,-1], na.rm=TRUE)
wshr <- data_shar[,-1]/rwsm
...
liqp_prst_rvrs <- temp_liqp_prst*0 # later on used for regression analysis

for (b in 1:length(temp_liqp_prst)){
  liqp_prst[b] <- sum(temp_liqp_prst[1:b])
  liqp_prst_rvrs[b] <- sum(temp_liqp_prst[b:length(liqp_prst)])
}

### Calculation of "subsequent" LP amount - later on used for regression analysis
liqp_amnt_rmng <- as_tibble(data_shar[,-1]*0)

for (i in 1:nrow(liqp_amnt_rmng)){
  for (j in 1:ncol(liqp_amnt_rmng)){
    liqp_amnt_rmng[i,j] <- liqp_prst_rvrs[(unlist(data_snry[,-1][i,j])+1)]
  }
}

liqp_rmng <- rowMeans(liqp_amnt_rmng, na.rm=TRUE)
...
scba_dsct_wght <- rowSums(scba_dsct*wshr, na.rm=TRUE)

### List of outputs
scba_outp <- list(Scenario=scnr,
                  Index=indx, 
                  Price=data_prce[,-1], 
                  Shares=data_shar[,-1], 
                  Total_Invest=sum(data_prce[,-1]*data_shar[,-1], na.rm=TRUE),
                  Share_Invest=rowSums(data_prce[,-1]*data_shar[,-1], na.rm=TRUE)/sum(data_prce[,-1]*data_shar[,-1], na.rm=TRUE),
                  Share_Shares=rowSums(data_shar[,-1], na.rm=TRUE)/sum(data_shar[,-1], na.rm=TRUE),
                  Investor_Family = data_fmly_eval,
                  Seniority=rowMeans(data_snry[,-1], na.rm=TRUE),
                  LP_Amounts=liqp_rmng,
                  LP_Allocated=liqp_allo,
                  Allocation_Base=data_allo,
                  FV_Value=temp_valu,
                  PMV_Value=pstm_valu,
                  Allocated_Amounts=scba,
                  Discount=scba_dsct_wght
)
...
shba_dsct_wght <- rowSums(shba_dsct*wshr, na.rm=TRUE)

### List of outputs
shba_outp <- list(Scenario=scnr,
                  Index=indx, 
                  Price=data_prce[,-1], 
                  Shares=data_shar[,-1], 
                  Total_Invest=sum(data_prce[,-1]*data_shar[,-1], na.rm=TRUE),
                  ...
                  FV_Value=temp_valu,
                  PMV_Value=pstm_valu,
                  Allocated_Amounts=shba,
                  Discount=shba_dsct_wght
)
...
cpra_dsct_wght <- rowSums(cpra_dsct*wshr, na.rm=TRUE)

### List of outputs
cpra_outp <- list(Scenario=scnr,
                  Index=indx, 
                  Price=data_prce[,-1], 
                  Shares=data_shar[,-1], 
                  Total_Invest=sum(data_prce[,-1]*data_shar[,-1], na.rm=TRUE),
                  ...
                  FV_Value=temp_valu,
                  PMV_Value=pstm_valu,
                  Allocated_Amounts=cpra,
                  Discount=cpra_dsct_wght
)
}
### Checks ####
